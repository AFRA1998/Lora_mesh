...
//esp_timer_handle_t hello_timer_handle;
...
void sendHelloPacketRoutine(void* parameter) {
    LoraMesher* instance = static_cast<LoraMesher*>(parameter); // Convertir le paramètre en instance
    if (instance != nullptr) {
        instance->runSendHelloPacketRoutine(); // Appeler la méthode membre
    }
}
...
//     /// Create the Hello Task if it hasn't been created yet
//     if (Hello_TaskHandle == NULL) {
//         xTaskCreate(sendHelloPacketRoutine, "HelloTask", 2048, this, 1, &Hello_TaskHandle);
//         }  // NOLINT
...
void LoraMesher::start() {
    ...
    //     xTimerReset(Hello_TaskHandle, 0);
...
LoraMesher::~LoraMesher() {
    ...
    // if (Hello_TaskHandle != nullptr)
    //     timer_delete( Hello_TaskHandle, 0);
...
void LoraMesher::initializeLoRa() {
    ...
    LoraMesherConfig config = *loraMesherConfig;
     // Forcer des broches SPI valides
    config.spiMiso = 19; // GPIO19
    config.spiMosi = 27; // GPIO23
    config.spiSclk = 5; // GPIO18
    config.loraCs = 18;   // GPIO5
    // Vérification des broches
    if (config.spiMiso < 0 || config.spiMiso > 39 ||
        config.spiMosi < 0 || config.spiMosi > 39 ||
        config.spiSclk < 0 || config.spiSclk > 39) {
        ESP_LOGE(LM_TAG, "Invalid SPI pin configuration: MISO: %d, MOSI: %d, SCLK: %d",
                 config.spiMiso, config.spiMosi, config.spiSclk);
        return;
    }
    ESP_LOGI("LoRaMesher", "SPI MISO: %d, MOSI: %d, SCLK: %d",
             config.spiMiso, config.spiMosi, config.spiSclk);
 ...
 // Initialisation SPI basée sur ESP-IDF
    spi_bus_config_t buscfg = {
        .mosi_io_num = config.spiMosi,
        .miso_io_num = (config.spiMiso != -1 ? config.spiMiso : -1),
    ...
    if (radio == nullptr) {
        //Module* mod = new Module(config.spiMosi, config.spiMiso, config.spiSclk, config.loraCs);
          EspHal* hal = new EspHal(config.spiSclk, config.spiMiso, config.spiMosi); // Create an instance of EspHal
          Module* mod = new Module(hal, config.loraCs, config.loraIrq, config.loraRst, config.loraIo1);
        switch (config.module) {
            case LoraModules::SX1276_MOD:
                ESP_LOGV(LM_TAG, "Using SX1276 module");
                // radio = new LM_SX1276(mod, config.loraIrq, config.loraRst);
                radio = new LM_SX1276(hal, config.loraCs, config.loraIrq, config.loraRst);

                break;
             case LoraModules::SX1262_MOD:
            ESP_LOGV(LM_TAG, "Utilisation du module SX1262");
            radio = new LM_SX1262(hal, config.loraCs, config.loraIrq, config.loraRst, config.loraIo1);
            break;

        case LoraModules::SX1278_MOD:
            ESP_LOGV(LM_TAG, "Utilisation du module SX1278");
            radio = new LM_SX1262(hal, config.loraCs, config.loraIrq, config.loraRst, config.loraIo1);
            break;

        case LoraModules::SX1268_MOD:
            ESP_LOGV(LM_TAG, "Utilisation du module SX1268");
            radio = new LM_SX1262(hal, config.loraCs, config.loraIrq, config.loraRst, config.loraIo1);
            break;

        case LoraModules::SX1280_MOD:
            ESP_LOGV(LM_TAG, "Utilisation du module SX1280");
            radio = new LM_SX1262(hal, config.loraCs, config.loraIrq, config.loraRst, config.loraIo1);
            break;

        default:
            ESP_LOGV(LM_TAG, "Module par défaut : SX1276");
            radio = new LM_SX1276(hal, config.loraCs, config.loraIrq, config.loraRst);
            break;
        }
    }
...
void LoraMesher::initializeSchedulers() {
    ...
    res = xTaskCreate(
        [](void* o) { static_cast<LoraMesher*>(o)->runSendHelloPacketRoutine(); },
        "Hello routine",
        4096,
        this,
        4,
...
// Délai pour laisser le timer fonctionner (facultatif)
...
void LoraMesher::runSendHelloPacketRoutine() {
    ...
    for (;;) {
        ...
        // Wait for the notification to send the Hello packet
        ...
        vTaskDelay(pdMS_TO_TICKS(HELLO_PACKETS_DELAY * 1000));
    }
}
...
void LoraMesher::sendHelloPacketTimer() {
    ...
    // Notify the Hello Task to send the packet
...
void LoraMesher::processDataPacket(QueuePacket<DataPacket>* pq) {
    ...
    if (packet->dst == getLocalAddress()) {
        ...
        ESP_LOGV(LM_TAG, "Data packet from %X for me, type %u", packet->src, (unsigned int)packet->type);
