...
uint8_t RoutingTableService::calculateMetric(uint8_t previous_metric, uint8_t hops, uint8_t rlq, uint8_t tlq) {
    ...
    if (hops == 1) {
        ...
        uint8_t factor_link_quality = LM_MAX_METRIC / std::sqrt(std::pow(LM_MAX_METRIC / std::max(previous_metric, uint8_t(1)), 2) + std::pow(LM_MAX_METRIC / std::max(quality_link, uint8_t(1)), 2) );
...
bool RoutingTableService::processHelloPacket(HelloPacket* p, int8_t receivedSNR, Packet<uint8_t>** out_send_packet) {
    ...
    if (p->routingTableId != routingTableId || p->routingTableSize != routingTableSize()) {
        ...
        ESP_LOGI(LM_TAG, "Hello packet from 0x%X with different RTId %d or RTSize %d\n", p->src, p->routingTableId, p->routingTableSize);
